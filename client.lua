...
RegisterNUICallback('uiLoaded', function(_, cb)
	...
	if UIConfig then SendNUIMessage({ action = 'setUIConfig', data = UIConfig }) end

	local saved = GetResourceKvpString('blossom_inv_settings')

	if saved then
		local ok, data = pcall(json.decode, saved)
		if ok and data then SendNUIMessage({ action = 'setUIConfig', data = data }) end
	end
...
RegisterNUICallback('saveSettings', function(data, cb)
	SetResourceKvp('blossom_inv_settings', json.encode(data))
	cb(1)
end)
