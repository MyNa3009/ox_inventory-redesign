...
import { hexToRgbArray } from '../../utils/hexToRgb';
...
const lightenColor = (hex: string) => {
  ...
  const m = (c: number) => Math.round(c + (255 - c) * 0.55);
...
const InventorySlot: React.ForwardRefRenderFunction<HTMLDivElement, SlotProps> = (
  ...
  const rarityTier = item?.name ? UIConfig.itemRarity[item.name] : undefined;
  const rarityColor = rarityTier ? UIConfig.rarities[rarityTier] : undefined;
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={rarityColor ? 'inventory-slot rarity' : 'inventory-slot'}
      style={
        {
          filter:
          ...
          ...(rarityColor
            ? {
                '--rarity': rarityColor,
                '--rarity-soft': `${rarityColor}66`,
                '--rarity-glow': `${rarityColor}73`,
                '--rarity-side': `${rarityColor}40`,
                '--rarity-text': lightenColor(rarityColor),
              }
            : {}),
